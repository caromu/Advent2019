...
std::tuple<std::tuple<int, int>, int> parseSegment(std::string seg)
{
    ...
    case 'L':
        ...
        break;
    default:
        throw "Unsupported Direction Prefix";
    }

    int length = std::stoi(seg.substr(1));

    return { dir, length };
}

void recordPath(std::tuple<int, int> &current, unsigned int &steps, std::tuple<int, int> dir, int length, short id)
{
    for (int i = 0; i < length; i++)
    {
        current = { std::get<0>(current) + std::get<0>(dir), std::get<1>(current) + std::get<1>(dir) };
        steps++;

        std::pair<CoordinateMap::iterator, bool> ret;
        ret = coord.insert(std::pair< std::tuple<int, int>, unsigned int*>(current, new unsigned int[2]{ 0,0 }));
        if (ret.first->second[id] == 0)
            ret.first->second[id] = steps;
    }
}

void traceWire(std::string input, short id)
{
    ...
    {
        // parse out each segment
        std::string segment = input.substr(i, input.find(",", i)-i);
        auto seg = parseSegment(segment);
        recordPath(pos, steps, std::get<0>(seg), std::get<1>(seg), id);
        i += (segment.length() + 1);
    }
}

int getManhattanDistance(std::tuple<int, int> pt)
{
    return abs(std::get<0>(pt)) + abs(std::get<1>(pt));
}
...
int main()
{
    ...
    for (auto i = coord.begin(); i != coord.end(); ++i) {
        if (i->second[0] != 0 && i->second[1] != 0) {
            int dist = i->second[0] + i->second[1];
            if (dist < minDist)
                minDist = dist;
        }
    }
